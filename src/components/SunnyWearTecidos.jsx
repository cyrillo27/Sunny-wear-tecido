...
const SunnyWearTecidos = () => {
  ...
  // 📦 Lote aberto via QR Code (?lote=LT-...)
  ...
  // ================== 📦 FUNÇÕES DE LOTE ==================
  const urlDoLote = (loteId) => `${window.location.origin}${window.location.pathname}?lote=${encodeURIComponent(loteId)}`;
  const urlQrDoLote = (loteId, tamanho = 260) =>
    `https://api.qrserver.com/v1/create-qr-code/?size=${tamanho}x${tamanho}&data=${encodeURIComponent(urlDoLote(loteId))}`;

  const handleFotoItemLote = (e) => {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => setItemLote((prev) => ({ ...prev, foto: reader.result }));
      reader.readAsDataURL(file);
    }
  };
  ...
  const adicionarItemAoLote = (e) => {
    e.preventDefault();
    const qtd = parseNumero(itemLote.quantidade);
    const local = (itemLote.localizacao || loteCabecalho.localizacao || '').trim();
    if (!itemLote.codigo.trim() || !itemLote.nome.trim() || !itemLote.cor.trim() || qtd <= 0) {
      alert('Preencha Código, Nome, Cor e Quantidade do tecido.');
      return;
    }
    if (!local) {
  ...
  const removerItemDoLote = (tmpId) => {
    setItensLote((prev) => prev.filter((i) => i._tmpId !== tmpId));
  };
  ...
  const limparLote = () => {
    ...
    setItemLote(itemLoteVazio);
    setTermoBuscaLote('');
    setLoteCabecalho({ codigoLote: gerarCodigoLote(), fornecedor: '', notaFiscal: '', localizacao: '', observacao: '' });
  };

  const finalizarLote = () => {
    const codigoLote = (loteCabecalho.codigoLote || '').trim().toUpperCase().replace(/[\[\]]/g, '');
    if (!codigoLote) {
    ...
    pedirSenha(`Autorização para salvar o lote ${codigoLote} com ${itensLote.length} tecido(s):`, async () => {
      setCarregando(true);
      const falhas = [];
      try {
        ...
        if (falhas.length === itensLote.length) {
          alert('❌ Nenhum tecido foi salvo. Verifique a conexão com o servidor.');
          return;
        }
        if (falhas.length > 0) {
  ...
  const lotesFiltrados = listaLotes.filter((g) => {
    ...
    return normalizarTexto(g.id).includes(termo) ||
      normalizarTexto(g.fornecedor).includes(termo) ||
      normalizarTexto(g.notaFiscal).includes(termo) ||
      g.itens.some((i) => normalizarTexto(i.codigo).includes(termo) || normalizarTexto(i.nome).includes(termo) || normalizarTexto(i.cor).includes(termo));
  });

  const imprimirEtiquetaLote = (loteId) => {
    const g = lotesAgrupados[loteId];
    if (!g) return;
    ...
    janela.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Lote ${esc(loteId)}</title>
      <style>body{font-family:Arial,sans-serif;padding:24px;color:#0F172A}h1{margin:0 0 4px;font-size:22px}p{margin:2px 0;font-size:13px;color:#475569}
      .topo{display:flex;gap:24px;align-items:center;margin-bottom:18px}table{width:100%;border-collapse:collapse;font-size:12px}
  ...
  if (loteQrUrl) {
    ...
    return (
      <div style={styles.qrViewContainer}>
        <div style={{ ...styles.qrViewCard, maxWidth: '560px' }}>
          <div style={{ textAlign: 'center', marginBottom: '16px' }}>
            <div style={styles.logoBadge}>SW</div>
            <h2 style={{ color: '#0F172A', margin: '10px 0 4px 0', fontSize: '20px', fontWeight: '800' }}>📦 Lote {loteIdUrl}</h2>
            <p style={{ color: '#2563EB', fontSize: '11px', margin: 0, fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px' }}>
              Consulta de Lote • Nuvem
            </p>
          </div>
    ...
    <div style={styles.appLayout} className="app-layout-container">
      ...
      <aside style={styles.sidebar}>
        ...
        <div style={styles.sidebarNavGroup}>
          ...
          <button 
            onClick={() => { setAbaAtiva('lote'); setMenuMobileAberto(false); }} 
            style={{ ...styles.sidebarLink, ...(abaAtiva === 'lote' ? styles.sidebarLinkActive : {}) }}
          >
            📦 Lote
          </button>
          <button 
          ...
          <>
            <div style={styles.cardSection}>
              ...
              <div style={styles.formGrid} className="form-grid-responsive">
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Código do Lote *</label>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <input type="text" value={loteCabecalho.codigoLote} onChange={(e) => setLoteCabecalho({ ...loteCabecalho, codigoLote: e.target.value.toUpperCase() })} style={{ ...styles.input, flex: 1 }} />
                    <button type="button" title="Gerar novo código" onClick={() => setLoteCabecalho({ ...loteCabecalho, codigoLote: gerarCodigoLote() })} style={{ ...styles.btnQr, margin: 0 }}>🔄</button>
                  </div>
                </div>
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Fornecedor</label>
                  <input type="text" placeholder="Ex: Têxtil Exemplo S.A." value={loteCabecalho.fornecedor} onChange={(e) => setLoteCabecalho({ ...loteCabecalho, fornecedor: e.target.value })} style={styles.input} />
                </div>
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Número da Nota Fiscal</label>
                  <input type="text" placeholder="Ex: 45892" value={loteCabecalho.notaFiscal} onChange={(e) => setLoteCabecalho({ ...loteCabecalho, notaFiscal: e.target.value })} style={styles.input} />
                </div>
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Localização Padrão do Lote</label>
                  <input type="text" placeholder="Ex: Galpão A - Setor 2" value={loteCabecalho.localizacao} onChange={(e) => setLoteCabecalho({ ...loteCabecalho, localizacao: e.target.value })} style={styles.input} />
                </div>
                <div style={{ ...styles.formGroup, gridColumn: '1 / -1' }}>
                  <label style={styles.formLabel}>Observação do Lote</label>
                  <input type="text" placeholder="Ex: Coleção Verão" value={loteCabecalho.observacao} onChange={(e) => setLoteCabecalho({ ...loteCabecalho, observacao: e.target.value })} style={styles.input} />
                </div>
              </div>

              {/* Adicionar tecido */}
              <form onSubmit={adicionarItemAoLote} style={{ ...styles.formGrid, marginTop: '20px', background: '#F8FAFC', padding: '16px', borderRadius: '12px', border: '1px solid #E2E8F0' }} className="form-grid-responsive">
                <div style={{ gridColumn: '1 / -1' }}>
                  <strong style={{ color: '#0F172A', fontSize: '14px' }}>➕ Adicionar tecido ao lote</strong>
                </div>
                      ...
                      {(() => {
                        ...
                        return resultados.map((t, idx) => (
                          <div
                            ...
                            style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: '#FFFFFF', border: '1px solid #BFDBFE', borderRadius: '8px', cursor: 'pointer' }}
                          >
                            <div>
                ...
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Largura (m)</label>
                  <input type="text" placeholder="Ex: 1.50" value={itemLote.largura} onChange={(e) => setItemLote({ ...itemLote, largura: e.target.value })} style={styles.input} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '10px' }}>
                  <div style={styles.formGroup}>
                    <label style={styles.formLabel}>Quantidade *</label>
                    <input type="text" placeholder="0.00" value={itemLote.quantidade} onChange={(e) => setItemLote({ ...itemLote, quantidade: e.target.value })} style={styles.input} />
                  </div>
                  <div style={styles.formGroup}>
                    <label style={styles.formLabel}>Unidade</label>
                    <select value={itemLote.unidadeMedida} onChange={(e) => setItemLote({ ...itemLote, unidadeMedida: e.target.value })} style={styles.input}>
                      <option value="m">Metros (m)</option>
                      <option value="kg">Quilos (kg)</option>
                    </select>
                  </div>
                ...
                <div style={styles.formGroup}>
                  ...
                  <input type="text" placeholder={loteCabecalho.localizacao || 'Ex: Galpão A - Setor 2'} value={itemLote.localizacao} onChange={(e) => setItemLote({ ...itemLote, localizacao: e.target.value })} style={styles.input} />
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={styles.formLabel}>Foto do Tecido (Opcional)</label>
                  <input type="file" accept="image/*" capture="environment" onChange={handleFotoItemLote} style={styles.inputFile} />
                  {itemLote.foto && (
                    <img src={itemLote.foto} alt="Prévia" style={{ ...styles.previewImg, marginTop: '8px' }} onClick={() => setFotoSelecionada(itemLote.foto)} />
                  )}
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <button type="submit" style={{ ...styles.button, background: '#2563EB', color: '#fff' }}>➕ Adicionar Tecido ao Lote</button>
                </div>
              </form>

              {/* Tecidos do lote atual */}
              <div style={{ marginTop: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px', marginBottom: '10px' }}>
                  <strong style={{ color: '#0F172A', fontSize: '14px' }}>🧵 Tecidos neste lote ({itensLote.length})</strong>
                  {itensLote.length > 0 && (
                    <span style={{ fontSize: '12px', color: '#64748B' }}>
                      Total: <strong>{arredondar(itensLote.filter(i => i.unidadeMedida !== 'kg').reduce((a, i) => a + parseNumero(i.quantidade), 0))} m</strong>
                      {itensLote.some(i => i.unidadeMedida === 'kg') && <> • <strong>{arredondar(itensLote.filter(i => i.unidadeMedida === 'kg').reduce((a, i) => a + parseNumero(i.quantidade), 0))} kg</strong></>}
                    </span>
                  )}
                </div>
                <div style={styles.tableResponsive}>
                  <table style={styles.table}>
                    ...
                    <tbody>
                      {itensLote.length === 0 ? (
                        <tr><td colSpan="6" style={styles.empty}>Nenhum tecido adicionado ainda.</td></tr>
                      ) : itensLote.map((it) => (
                        <tr key={it._tmpId} style={styles.tr}>
                          <td style={styles.td}><strong style={{ color: '#0F172A' }}>{it.codigo}</strong></td>
                          <td style={styles.td}>
                            <div style={{ fontWeight: '600', color: '#0F172A' }}>{it.nome} <span style={{ color: '#2563EB' }}>({it.cor})</span></div>
                            {it.largura ? <div style={{ fontSize: '11px', color: '#2563EB' }}>Largura: {it.largura}m</div> : null}
                          </td>
                          <td style={styles.td}><strong>{it.quantidade} {it.unidadeMedida}</strong></td>
                          <td style={styles.td}>{parseNumero(it.preco) > 0 ? `R$ ${(parseNumero(it.preco) * parseNumero(it.quantidade)).toFixed(2)}` : '-'}</td>
                          <td style={styles.td}><span style={styles.localBadge}>📍 {it.localizacao}</span></td>
                          <td style={styles.td}>
                            <button onClick={() => removerItemDoLote(it._tmpId)} style={styles.btnDeletar} title="Remover do lote">🗑️</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '16px', flexWrap: 'wrap' }}>
                  <button
                    type="button"
                    onClick={() => { if (itensLote.length === 0 || window.confirm('Descartar os tecidos adicionados neste lote?')) limparLote(); }}
                    style={{ ...styles.button, flex: 1, background: '#F1F5F9', color: '#475569', boxShadow: 'none', border: '1px solid #CBD5E1' }}
                  >
                    Limpar
                  </button>
                  <button
            ...
            {/* Lotes já cadastrados */}
            <div style={{ ...styles.cardSection, marginTop: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
                <h3 style={{ ...styles.sectionTitle, margin: 0 }}>📋 Lotes Cadastrados</h3>
                <span style={{ fontSize: '12px', color: '#64748B' }}>Total: <strong>{listaLotes.length}</strong></span>
              </div>
              <input
                type="text"
                placeholder="Pesquisar por código do lote, fornecedor, NF ou tecido..."
                value={buscaLotesTexto}
                onChange={(e) => setBuscaLotesTexto(e.target.value)}
                style={styles.inputFull}
              />
              <div style={styles.tableResponsive}>
                <table style={styles.table}>
                  ...
                  <tbody>
                    {lotesFiltrados.length === 0 ? (
                      <tr><td colSpan="6" style={styles.empty}>Nenhum lote encontrado.</td></tr>
                    ) : lotesFiltrados.map((g) => (
                      <tr key={g.id} style={styles.tr}>
                        <td style={styles.td}><strong style={{ color: '#7C3AED' }}>📦 {g.id}</strong></td>
                        <td style={styles.td}>
                          <div style={{ fontWeight: '700', color: '#0F172A' }}>{g.itens.length} tecido(s)</div>
                          <div style={{ fontSize: '11px', color: '#64748B', maxWidth: '260px' }}>
                            {g.itens.slice(0, 3).map(i => `${i.nome} (${i.cor})`).join(', ')}{g.itens.length > 3 ? '...' : ''}
                          </div>
                        </td>
                        <td style={styles.td}>
                        ...
                        <td style={styles.td}>
                          ...
                          <button onClick={() => excluirLote(g.id)} style={styles.btnDeletar} title="Excluir lote">🗑️</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {loteSelecionado && (() => {
        const g = lotesAgrupados[loteSelecionado];
        return (
          <div style={styles.modalOverlay} onClick={() => setLoteSelecionado(null)}>
            <div style={{ ...styles.modalContent, alignItems: 'center', width: '100%', maxWidth: '520px', maxHeight: '90vh', overflowY: 'auto', margin: 'auto' }} onClick={(e) => e.stopPropagation()}>
              <button style={styles.modalCloseBtn} onClick={() => setLoteSelecionado(null)}>✕ Fechar</button>
              <h3 style={{ margin: '0 0 4px 0', fontSize: '18px', color: '#0F172A', fontWeight: '800' }}>📦 Lote {loteSelecionado}</h3>
              {!g ? (
                <p style={{ fontSize: '13px', color: '#64748B' }}>{carregando ? 'Carregando lote...' : 'Lote não encontrado (aguarde a sincronização).'}</p>
              ) : (
                <>
